...
uint8_t buffer_row[81];
extern mode_e mode;
extern switch_e actionOpenClose;
...
void write_byte(uint8_t byte)
{
  for (int i = 0; i < 8; i++)
  {
    ...
    digitalWrite(data_Pin, byte & 0x80);
    byte = byte << 1;
...
void scroll_string(uint8_t *string, uint32_t speed, char direction)
{
  ...
  Serial.println(readValueBt);

  
  for (int i = 0; i < len; i++)
  {
    readValueBt = ((((uint8_t)mode) << 2) | (((uint8_t)actionOpenClose) << 1) | ((uint8_t)resetMode));
    //actionOpenClose = getStatusButton(&buttonSwitch);
    if(oldValue != readValueBt)
    {
      oldValue = readValueBt;
      max_clear();
      break;
    }
